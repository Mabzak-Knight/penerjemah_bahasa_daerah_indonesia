...
def load_dictionary_from_csv(file_path):
    ...
    with open(file_path, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            jawa_word = row[0]
            indonesia_translation = row[1]
            dictionary[jawa_word] = indonesia_translation
    return dictionary

def translate_jawa_to_indonesia(text, dictionary):
    words = text.split()
    translated_words = []
    for word in words:
...
# Memuat kamus dari file CSV
csv_file_path = 'kamus_jawa_indonesia.csv'
dictionary = load_dictionary_from_csv(csv_file_path)

# Contoh penggunaan
input_text = 'aba-aba (ta) abab (ta) abang (ts)'
translated_text = translate_jawa_to_indonesia(input_text, dictionary)
